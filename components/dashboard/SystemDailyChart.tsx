...
export default function SystemDailyChart({ range }: { range: string }) {
  ...
  useEffect(() => {
    fetch(`/api/dashboard/system-daily?range=${range}`, { credentials: "include" })
      .then(r => r.json())
      .then(json => {
        const data = json.series ?? json.days ?? [];
        setRawData(data);
        setTotal(json.todayTotal ?? 0);
        setLoading(false);
      })
  ...
  const maxPoints = Math.max(...rawData.map((b: any) => Number(b.points ?? b.pointsEarned ?? 0)), 1);
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="h-32">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={rawData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <defs>
            ...
            <Tooltip
              ...
              formatter={(value: any) => [`${Number(value || 0).toLocaleString()} pts`, "System"]}
            />
