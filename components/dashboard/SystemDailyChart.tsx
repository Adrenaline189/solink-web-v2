...
type DailyBucket = { label: string; points: number };
...
export default function SystemDailyChart({ range }: { range: string }) {
  ...
  useEffect(() => {
    const url = `/api/dashboard/system-daily?range=${range}`;
    fetch(url, { credentials: "include" })
      .then(r => r.json())
      .then(json => {
        const series = json.series ?? json.days ?? [];
        const mapped: DailyBucket[] = series.map((b: any) => ({
          label: b.label ?? "",
          points: b.points ?? b.pointsEarned ?? 0,
        }));
        setBuckets(mapped);
        setTotal(json.todayTotal ?? mapped.reduce((s: number, x: DailyBucket) => s + x.points, 0));
        setLoading(false);
      })
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-xs text-slate-400">
        System: {total.toLocaleString()} pts ({range})
      </div>
      <div className="h-32">
